...
import { getOrders, getOrder, createOrder, deleteOrder, updateOrder, getProducts, getOrderPositions, getPositionActions, addPosition, deletePosition, updateOrderStatus, bulkUpdateOrderStatus, updateAction, deleteAction } from '../api';
...
export default function AdminOrders() {
    ...
    const handleUpdateAction = async (actionId: number, actionType: ActionType, positionId: number) => {
        ...
        try {
            await updateAction(user.id, actionId, { action_type: actionType, quantity: qty });
            // Refresh
                                            ...
                                            {filteredOrders.map(order => (
                                                <React.Fragment key={order.id}>
                                                    <tr>
                                                        ...
                                                        <td>{formatDate(order.expected_shipment_date)}</td>
                                                    ...
                                                    {order.positions.map(pos => (
                                                        <tr key={`pos-${pos.id}`} className="position-subrow">
                                                            ...
                                                            <td colSpan={2}>
                                                                <div className="subrow-stats">
                                                                    {(['cutting', 'sewing', 'ironing', 'packing'] as ActionType[]).map(at => {
                                                                        const done = pos.action_totals[at] || 0;
                                                                        const isComplete = done >= pos.quantity;
                                                                        return (
                                                                            <span key={at} className={`stat-mini ${isComplete ? 'complete' : ''}`}>
                                                                                {ACTION_TYPE_LABELS[at].charAt(0)}: {done}/{pos.quantity}
                                                                            </span>
                                                                        );
                                                                    })}
                                                                </div>
