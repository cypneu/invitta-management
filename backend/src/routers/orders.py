...
def list_orders_for_worker(
    ...
    """List orders for workers - only in_progress and done orders with positions."""
    from .actions import get_action_totals
    ...
    # Workers can only see in_progress and done orders
    query = query.filter(Order.status.in_([OrderStatus.in_progress, OrderStatus.done]))
